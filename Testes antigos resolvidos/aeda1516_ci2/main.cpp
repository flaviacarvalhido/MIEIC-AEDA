...
void test_C_setIngredients() { //TODO
    ...
    is1 = m1.getIngredients();
    ASSERT_EQUAL( i = 5, is1.size());

}

void test_D_optionsWithinIngredientLimits() { //TODO

    cout << endl << "Alinea d:" << endl;
    ...
    try {
        ASSERT_EQUAL(i=2, marios.optionsWithinIngredientLimits(3, 5).size());
        ASSERT_EQUAL(i=0, marios.optionsWithinIngredientLimits(1, 2).size());
        ASSERT_EQUAL(i=0, marios.optionsWithinIngredientLimits(10, 15).size());
        ASSERT_EQUAL(i=1, marios.optionsWithinIngredientLimits(7, 7).size());
    }
...
void test_E_popularityRank() { //TODO
    ...
    ASSERT_EQUAL(str="chicken", rank[0].getName());
    ASSERT_EQUAL(str="tuna", rank[3].getName());

}

void test_F_chefCustomer() { //TODO

    cout << endl << "Alinea f:" << endl;
...
void test_G_removeIngredient() { //TODO
    ...
    ASSERT_EQUAL(i=2, m1.getIngredients().size());

    m1 = marios.removeIngredient(is1, "olive");
    ASSERT_EQUAL(i=4, m1.getIngredients().size());

    m1 = marios.removeIngredient(m1.getIngredients(), "mozzarella");
    ASSERT_EQUAL(i=3, m1.getIngredients().size());
    ...
    try {
        ...
        ASSERT_EQUAL(s="tuna", e.getIngredient());
    }
    ...
    try {
        ...
        ASSERT_EQUAL(s="pepperoni", e.getIngredient());
    }
...
void all_tests() {
    test_A_constructorMenu();
    test_B_templateIsSet();
    test_C_setIngredients();
    test_D_optionsWithinIngredientLimits();
    test_E_popularityRank();
    test_F_chefCustomer();
    test_G_removeIngredient();
}
